...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="soft-grid min-h-screen bg-[#f7f7ff] text-[#18192b]">
        <section className="distribution-glow text-white">
          <div className="mx-auto grid max-w-[1320px] gap-8 px-4 py-14 sm:px-6 lg:grid-cols-[1fr_.8fr] lg:items-end lg:px-8 lg:py-20">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.24em] text-[#ffd45a]">Media directory</p>
              <h1 className="mt-4 text-5xl font-black leading-[.94] tracking-[-0.06em] sm:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
              <p className="mt-6 max-w-xl text-base leading-8 text-white/70">Find fresh releases, announcements, company stories and public updates ready to explore.</p>
            </div>
            <form action="/search" className="flex rounded-xl bg-white p-1.5 shadow-2xl"><Search className="ml-3 mt-3 h-5 w-5 text-[#777]"/><input name="q" placeholder="Search the distribution desk" className="min-w-0 flex-1 bg-transparent px-3 text-sm text-[#222] outline-none"/><button className="rounded-lg bg-[#ff5a5a] px-5 text-sm font-black">Search</button></form>
          </div>
        </section>

        <section className="border-b border-[#e5e4f3] bg-white">
          <div className="mx-auto flex max-w-[1320px] gap-3 overflow-x-auto px-4 py-5 text-xs font-bold sm:px-6 lg:px-8">
            <Link href={basePath} className={`whitespace-nowrap rounded-full px-4 py-2 ${category === 'all' ? 'bg-[#3b00f5] text-white' : 'bg-[#f2f1ff]'}`}>All media</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={`whitespace-nowrap rounded-full px-4 py-2 ${category === item.slug ? 'bg-[#ff5a5a] text-white' : 'bg-[#f2f1ff] hover:bg-[#e9e7ff]'}`}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1320px] gap-5 px-4 py-10 sm:px-6 lg:grid-cols-[1.55fr_.75fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden rounded-[1.75rem] bg-[#2500bd] shadow-[0_22px_65px_rgba(42,17,128,.16)]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[#ff5a5a] px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em]">{getCategory(lead, label)}</span>
            ...
            <div className="grid gap-4">
              <div className="rounded-2xl bg-[#ff5a5a] p-6 text-white">
              ...
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] overflow-hidden rounded-2xl bg-white shadow-sm transition hover:-translate-y-1">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#ff5a5a]">0{index + 1}</p>
        ...
        <section className="mx-auto max-w-[1320px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5">
            <div><p className="text-xs font-black uppercase tracking-[.2em] text-[#ff5a5a]">Browse the feed</p><h2 className="mt-2 text-4xl font-black tracking-[-0.05em] sm:text-5xl">Latest distribution</h2></div>
            <form action={basePath} className="flex overflow-hidden rounded-xl border border-[#deddec] bg-white">
              <select name="category" defaultValue={category} className="h-12 min-w-44 bg-transparent px-3 text-xs font-bold outline-none">
                <option value="all">All categories</option>
                {categories.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
              </select>
              <button className="h-12 bg-[#3b00f5] px-5 text-xs font-black text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className={`group overflow-hidden bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${index % 3 === 0 ? 'rounded-[1.8rem]' : index % 3 === 1 ? 'rounded-xl border border-[#e6e5f0]' : 'rounded-[1.8rem_1.8rem_.5rem_1.8rem]'}`}>
                  <div className={`${index % 3 === 1 ? 'aspect-[4/3]' : 'aspect-[16/10]'} overflow-hidden bg-[#ececf8]`}>
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.18em] text-[#ff5a5a]">
                      <span>{getCategory(post, label)}</span><span>{String(index + 3).padStart(2, '0')}</span>
                    </div>
                    <h3 className="mt-4 text-2xl font-black leading-[1.05] tracking-[-.035em]">{post.title}</h3>
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            ...
            <span className="rounded-xl bg-[#3b00f5] px-5 py-3 text-xs font-black text-white">Page {page} / {pagination.totalPages || 1}</span>
